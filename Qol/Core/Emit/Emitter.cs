...
namespace Qol.Core.Emit
{
    internal sealed class Emitter
    {
        
        private DiagnosticBag _diagnostics = new DiagnosticBag();
        private Dictionary<FunctionSymbol, MethodDefinition> _methods;
        ...
        {
            var type = _knownTypes[function.Type];
        ...
        private void EmitUnaryExpression(ILProcessor ilProcessor, BoundUnaryExpression node)
        {
            ...
            {
                throw new Exception($"Unexpected unary operator type {SyntaxFacts.GetText(node.Op.SyntaxKind)}({node.Operand.Type})");
            }
        ...
        private void EmitBinaryExpression(ILProcessor ilProcessor, BoundBinaryExpression node)
        {
            ...
            switch (node.Op.Kind)
            {
                ...
                case BoundBinaryOperatorKind.GreaterOrEquals:
                    ...
                    break;
                    default:
                    throw new Exception($"Unexpected binary operator {SyntaxFacts.GetText(node.Op.SyntaxKind)}{node.Left.Type} {node.Op.Kind} {node.Right.Type}");
            }
        }
        ...
        {
            _randomFieldDefinition = new FieldDefinition("$random", FieldAttributes.Static | FieldAttributes.Private, _randomReference);
            _typeDefinition.Fields.Add(_randomFieldDefinition);


            var staticConstructor = new MethodDefinition(".cctor", MethodAttributes.Static | MethodAttributes.Private | MethodAttributes.SpecialName | MethodAttributes.RTSpecialName, _knownTypes[TypeSymbol.Void]);
