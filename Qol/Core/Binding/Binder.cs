...
namespace Qol.Core.Binding
{
    internal sealed class Binder
    {
        ...
        public static BoundGlobalScope BindGlobalScope(bool isScript, BoundGlobalScope previous, ImmutableArray<SyntaxTree> syntaxTrees)
        {
            ...
            // Remove this to allow global variables
            ...
            // These two lines are what defines a main function (which can be used with global variables) as long as the above and lower are removed.
            ...
            else
            {
                ...
                // This code disabled Global Variables in the entire project when using a main function.
                // which is not normal behavior for programming languages.
                // Try to remove this as soon as possible if he doesn't revert this change or fix global (static) variables.
                // which are now useless in the terms of having a main (autorun) style function like golang, or ctor like C#.
                ...
                if(globalStatements.Any())
                {
                    ...
                    {
                        // Initiate the main function for autorun. Keep this code below and remove it below MainFunction for use of globals and main.
                        mainFunction = new FunctionSymbol("main", ImmutableArray<ParameterSymbol>.Empty, TypeSymbol.Void, null);
                    }
                }
                // <--- end of code that renders main function useless with global variables.
            }
        ...
        private BoundStatement BindGlobalStatement(StatementSyntax syntax, bool isGlobal = false)
        {
        ...
        private BoundStatement BindReturnStatement(ReturnStatementSyntax syntax)
        {
            ...
            if (_function == null)
            {
                if(_isScript)
                {
                    // Ignore, because we allow return with or without values in script mode.
                    if(expression == null)
                        expression = new BoundLiteralExpression("");
                }
                else if(expression != null)
                {
                    // Main function does not support return values
                    _diagnostics.ReportInvalidReturnWithGlobalStatements(syntax.Expression.Location);
                }
            }
            else
            {
